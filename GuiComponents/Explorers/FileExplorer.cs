...
namespace KFA.GUI.Explorers {
	...
	public partial class FileExplorer : UserControl, IExplorer {
		...
		public FileExplorer() {
			...
			treeView1.SelectedImageKey = null;
		}
		...
		private static void AppendChildren(TreeNode node, IEnumerable<IFileSystemNode> children) {
			...
			foreach (IFileSystemNode child in children) {
				...
				} else if (child is KFS.FileSystems.File || child is HiddenDataStreamFileNTFS) {
					treeNode.ImageKey = "File";
				} else {
					treeNode.ImageKey = "Directory";
				}

				if (child is Folder || (child is KFS.FileSystems.File && ((KFS.FileSystems.File)child).IsZip)) {
			...
			if (stream != _currentStream) {
				...
				if (store != null) {
					...
					if (fs != null) {
						...
						treeView1.Nodes.Add("Unknown file system");
					}
				}
			}
		}
		...
		private void ShowContextMenu(TreeNodeMouseClickEventArgs e) {
			...
			if (fsnode != null) {
				...
				if (fsnode is KFS.FileSystems.File) {
                    KFS.FileSystems.File f = fsnode as KFS.FileSystems.File;
