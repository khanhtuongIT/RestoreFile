	...
	public class FileSystemNTFS : FileSystem {
		...
		internal override FileRecoveryStatus GetChanceOfRecovery(FileSystemNode node) {
			...
			if (file == null) {
                ...
                if (runs == null)
                {
					...
					if (usedClusters == 0) {
						return FileRecoveryStatus.MaybeOverwritten; // trả về trạng thái có thể ghi đè
					} else if (usedClusters < totalClusters) {
						return FileRecoveryStatus.PartiallyOverwritten; // trả về trạng thái ghi đè một phần
					} else {
						return FileRecoveryStatus.Overwritten; // trả về trang thái đã ghi đè
					}
				}
			}
		}
		...
		private void MftScan(FileSystem.NodeVisitCallback callback) {
			...
			for (ulong i = 0; i < numFiles; i++) {
				MFTRecord record = MFTRecord.Load(i, this, MFTLoadDepth.Full);
