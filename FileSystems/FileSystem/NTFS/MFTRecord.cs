...
namespace KFS.FileSystems.NTFS {
    ...
    public class MFTRecord : INodeMetadata {
        ...
        public DateTime LastAccessed { get { return LastAccessTime; } }
        ...
        private void LoadStandardAttribute(int startOffset) {
            CreationTime = fromNTFS(BitConverter.ToUInt64(_data, startOffset + 16));
                        ...
                        foreach (MFTAttribute externalAttribute in extensionRecord._attributes) {
                            if (id == externalAttribute.Id) {
                                if (externalAttribute.NonResident && externalAttribute.Type == AttributeType.Data) {
                                    // Tìm thuộc tính dữ liệu tương ứng trên bản ghi này và hợp nhất các danh sách chạy
                                    bool merged = false;
        ...
        private void MergeRunLists(ref List<IRun> list1, List<IRun> list2)
        {
            try
            {
                ...
                //    Debug.WriteLine("add range list successfull");
            }
            catch { Debug.WriteLine("add range list error"); }
		...
		public FileRecoveryStatus ChanceOfRecovery {
			get {
				if (_chanceOfRecovery == FileRecoveryStatus.Unknown || _chanceOfRecovery != FileRecoveryStatus.Unknown) {
