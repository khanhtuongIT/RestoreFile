...
{
    public partial class DeleteFileViewName : UserControl
    {
        ...
        private HashSet<string> _systemFileExtensions =
               new HashSet<string>() { ".TMP", ".CAB", ".LNK", ".LOG", ".EXE", ".XML", ".INI" };
        ...
        public DeleteFileViewName(Scanner scanner)
        {
            InitializeComponent();
        ...
        public void state_ScanFinished(object sender, EventArgs ea)
        {
            try
            {
                ...
                lvi = null;
                //_files.Clear();
                FrmFileName.Process_Status = 1;
                if (FrmFileName.Status)
                {
                    Thread.Sleep(1000);
                    progressBar.Hide();
                }
        ...
        public void HideProgressBarName()
        {
            Thread.Sleep(1000);
            progressBar.Hide();
        }
        ...
        public void SetScanButtonFinished()
        {
            ...
            FrmFileName.IsSelected = false;
            if (_isChecked == false)
            {
                FrmFileName.Instance.FileNameScanned = "Finished Scanning...!";
                FrmFileName.Instance.btnScan.Text = "Scan";
                FrmFileName.Instance.btnScan.Tag = "scan";
        ...
        private void SetProgress(double progress)
        {
            ...
            FrmFileName.Instance.ProgressValue = progressBar.Value;
        }
            ...
            if (e.Button == MouseButtons.Right)
            {
                ...
                if (fileView.SelectedItems.Count == 1)
                {
                    ...
                    if (metadata != null)
                    {
                        ...
                        MenuItem recoverFile = new MenuItem(strButton, new EventHandler(delegate (object o, EventArgs ea) {
                            ...
                            if (check)
                            {
                                if (_progressPopup.Visible) { FrmFileName.Instance.MessageBox(""+Properties.Resources.strMessage1); }
                                else
                                {
                ...
                else if (fileView.SelectedItems.Count > 1)
                {
                    ...
                    MenuItem recoverFiles = new MenuItem(strButton1, new EventHandler(delegate (object o, EventArgs ea) {
                        ...
                        if (check)
                        {
                            if (_progressPopup.Visible) { FrmFileName.Instance.MessageBox(_message1); }
                            else
                            {
        ...
        private void bRestoreFiles_Click(object sender, EventArgs e)
        {
            ...
            if (check)
            {
                if (_progressPopup.Visible)
                {
                    FrmFileName.Instance.MessageBox(""+Properties.Resources.strMessage1);
                }
                else
                {
        ...
        private void DeleteFileViewName_Leave(object sender, EventArgs e)
        {
            // _Isscan = false;
            scanAll = false;
        }
