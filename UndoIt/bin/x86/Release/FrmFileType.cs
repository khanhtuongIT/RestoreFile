...
{
    public partial class FrmFileType : UserControl
    {
        private static FrmFileType _instance;
        public static FrmFileType Instance
        {
            get
            {
                if (_instance == null)
                    _instance = new FrmFileType();
                return _instance;
            }
        }
        ...
        public string FileTypeScanned
        {
            set { lbfinetype.Text = value; }
        }
        ...
        public Dictionary<IFileSystem, DeleteFileViewType> _deletedViewers = new Dictionary<IFileSystem, DeleteFileViewType>();
        ...
        #region Form event
        private void FrmFileType_Load(object sender, EventArgs e)
        {
        ...
        private void FrmFileType_Leave(object sender, EventArgs e)
        {
            GC.Collect();
        }
        #endregion
        ...
        public void SetFileSystem(IFileSystemStore logicalDisk)
        {
            if (logicalDisk.FS != null)
            {
                if (!_scanners.ContainsKey(logicalDisk.FS))
                {
                    ...
                    _deletedViewers[logicalDisk.FS] = new DeleteFileViewType(_scanners[logicalDisk.FS]);
        ...
        private void AddDeletedFileViewer(DeleteFileViewType viewer)
        {
            int MARGIN = 12;
                    ...
                    if (process_status != 1)
                    {
                        if (jv == 0)
                        {
                            ...
                            if (FileSystemStore.ElementAt(iv) != null)
                            {
                                ...
                                Scanner.FileType = this.Type;
                                SetFileSystem(FileSystemStore.ElementAt(iv));
                                _scanners[FileSystemStore.ElementAt(iv).FS].StartScanType();
                            }
                            else
                            {
                                process_status = 1;
        ...
        private void btnScan_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                else
                {
                    try
                    {
                        DeleteFileViewType.IsChecked = false;
                        ...
                        _scanners[FileSystemStore.ElementAt(0).FS].StartScanType();
                        // _deletedViewers[item.FS].state_ScanFinished(sender, e);
                    }
                    catch
                    {
        ...
        public void StopScanningWhenFormLeave(object sender, EventArgs e)
        {
            if (drive == false)
            {
                ...
                foreach (var item in FileSystemStore)
                {
                    ...
                    catch { btnScan.Tag = "scan"; btnScan.Text = scan; lbfinetype.Text = finished; }
                }
            }
            else
            {
